...
__global__ void reduceUnrolled2(int *g_inputData, int *g_outputData, unsigned int n) {
    unsigned int tid = threadIdx.x;
    unsigned int bidx = blockDim.x * blockIdx.x * 2;
...
int main() {
    ...
    iStart = seconds();
    reduceUnrolled2<<<grid, block>>>(d_inputData, d_outputData, size);
    cudaDeviceSynchronize();
    iElapsed = seconds() - iStart;
